...
// Improved display function with fixed column widths
void displayData(EmploymentData data[], int count) {
    printf("Rank  %-35s %s\n", "Industry", "Employees");
    printf("----- %-35s %s\n", "-----------------------------------", "-----------");
    
    for (int i = 0; i < count; i++) {
        // Fixed format: rank (5), industry (35), employees (11)
        printf("%-5d %-35s %11d\n", 
               i + 1, 
               data[i].industry, 
               data[i].employees);
...
int main() {
    ...
    printf("\nSORTED EMPLOYMENT DATA (ALL %d RECORDS):\n", count);
    printf("==========================================\n");
    displayData(main_data_set, count);
    
    // Write full results to file
    FILE *output = fopen("employ-sort.txt", "w");
    if (output != NULL) {
        fprintf(output, "Rank  %-35s %s\n", "Industry", "Employees");
        fprintf(output, "----- %-35s %s\n", "-----------------------------------", "-----------");
        
        for (int i = 0; i < count; i++) {
            fprintf(output, "%-5d %-35s %11d\n", 
                    i + 1, 
    ...
    do {
        ...
        if (!found) {
            printf("\n✗ Not found: %s\n", search_industry);
            printf("Check the complete list above for correct industry names.\n");
        }
